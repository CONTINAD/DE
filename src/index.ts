...
async function main() {
  logger.info("=== $XStocks — claim SOL fees → buy the rotating stock → airdrop it to every holder → repeat ===");
  ...
  logger.info(`Cycle:             every ${config.cycleIntervalSeconds}s · rotation of ${ROTATION.length} stocks`);
  logger.info(`Rotation:          ${ROTATION.map((s) => s.ticker).join(" → ")}`);
  logger.info(`Qualify:           hold ≥ ${config.minHolderBalance.toLocaleString()} $XSTOCKS`);
  ...
  if (tracker.resetClaimPoolOnce("v2-fresh-fees")) {
    logger.info("Pool reset to 0: pre-fix accumulated SOL will NOT be spent; only fresh claims from now accumulate.");
  }
  ...
  if (!xstocksMintStr) {
    ...
    tracker.recordInfo(`Watching ${treasury.publicKey.toBase58()} for the pump.fun $XSTOCKS launch…`);
    ...
    tracker.recordInfo(`Detected $XSTOCKS mint: ${xstocksMintStr} — the machine is online.`);
    tracker.setIdentity({
      treasuryWallet: treasury.publicKey.toBase58(),
      marketingWallet: config.marketingWallet || "",
      xstocksMint: xstocksMintStr,
  ...
  const runCycle = async () => {
    ...
    try {
      ...
      // Spend (100 - reserve)% of the accumulated claimed-fee pool, so the bot
      // can build up enough to cover the one-time cost of opening holder token
      // accounts and actually distribute. The pool only ever holds CLAIMED fees
      // (never dev-wallet principal), and MAX_SPEND_PER_CYCLE_SOL caps any single
      // cycle so it can never dump a large amount at once.
      ...
      const capLamports = Math.floor(config.maxSpendPerCycleSol * LAMPORTS_PER_SOL);
      if (capLamports > 0 && dispensable > capLamports) {
        logger.info(`Spend capped at ${config.maxSpendPerCycleSol} SOL/cycle (pool ${poolSol.toFixed(5)} SOL).`);
        dispensable = capLamports;
      }
      ...
      if (qualified.length === 0) {
        tracker.recordInfo(`No wallets hold ≥ ${config.minHolderBalance.toLocaleString()} $XSTOCKS yet — pool carries over.`);
        return;
      }
