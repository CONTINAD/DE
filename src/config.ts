...
const mintRaw = process.env.XSTOCKS_MINT?.trim();
...
export const config = {
  ...
  // the pump.fun $XSTOCKS mint, claims creator fees (SOL), buys the rotating
  // xStock on Jupiter, and funds the airdrop. Every relay wallet is freshly
  ...
  // The project's own pump.fun token ($XSTOCKS). auto = watch the treasury and
  ...
  // ── The mechanic ──────────────────────────────────────────────────────
  // Every CYCLE_INTERVAL_SECONDS: claim SOL fees → buy the NEXT stock in the
  // rotation → airdrop it to EVERY wallet holding ≥ MIN_HOLDER_BALANCE $XSTOCKS,
  ...
  // Qualification threshold — only wallets holding at least this many $XSTOCKS
  // (UI amount) get paid. Pay EVERY wallet that clears this bar, every cycle.
